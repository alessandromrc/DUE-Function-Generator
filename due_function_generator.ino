//----------------------------------------------------------------------------//
/*
 *  Copyright (C) 2022 alessandromrc - Alessandro Marcon
 *
...
void setup() {
  ...
  attachInterrupt(pushButtonPin, ChangeWaveForm, RISING);
}

unsigned long previous_timer_val = 0;

void loop() {
  unsigned long Timer = micros();
  if (!(Timer - previous_timer_val > constrain(encoderValue, 0, oneHzSample)))
  {
    return;
  }
    analogWrite(DAC0, waveformsTable[waveType][i]);
    i = (i + 1) % maxSamplesNum;
    previous_timer_val = Timer;
}
...
void updateEncoder() {
  ...
  if (sum == 2) encoderValue++;
  if (sum == 8) encoderValue--;
  lastEncoded = encoded;
}
